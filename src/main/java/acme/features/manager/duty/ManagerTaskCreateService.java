...
@Service
public class ManagerTaskCreateService implements AbstractCreateService<Manager, Duty> {
	...
	public void validate(final Request<Duty> request, final Duty entity, final Errors errors) {
		...
		errors.state(request, spam1, "title","manager.duty.error.spam");
		final boolean spam2= !this.spamService.filtroSpam(entity.getDescription(),threshold);
		errors.state(request, spam2, "description","manager.duty.error.spam");
		...
		if(startMoment == null) {
			...
			errors.state(request, fechaInicioBien, "startMoment","manager.duty.error.startMoment");
		}
		...
		errors.state(request, fechaFinBien, "endMoment","manager.duty.error.endMoment");
		}
		//Validacion workload
		...
		if(workload == null || endMoment == null || startMoment == null) {
			...
			errors.state(request, false, "workload","manager.duty.error.workloadNegative");
	} 
			else if(workload > 99.59) {
			errors.state(request, false, "workload","manager.duty.error.workloadMax");
	} else if(fPartToInt.compareTo(minutosMax) == 1) {
		errors.state(request, false, "workload","manager.duty.error.workloadMaxMinutes");
	} else if(workloadInt > 99) {
		errors.state(request, false, "workload","manager.duty.error.workloadMaxHours");
	}
		else{ 
		...
		errors.state(request, workloadCorrecto, "workload","manager.duty.error.workload");
		}
